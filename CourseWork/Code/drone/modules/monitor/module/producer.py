...
def producer_job(_, config, requests_queue: multiprocessing.Queue):
    ...
    def delivery_callback(err, msg):
        if err:
            ...
            print(
                f"[kafka] monitor routed to {msg.topic()} "
                f"(id={msg.key().decode('utf-8') if msg.key() else '?'})"
            )
    ...
    while True:
        ...
        print(
            f"[kafka] monitor routing "
            f"{event_details.get('source')}->{topic}: "
            f"{event_details.get('operation')} (id={event_details.get('id')})"
        )
        producer.produce(
            topic,
