...
def simulate_move_shelf_and_station(robot, route_to_shelf, route_to_station, distances_to_shelf, distances_to_station):
    ...
    if robot.coordinates_qr != route_to_shelf[-1]:
        robot.is_move_shelf = False
        return "Не удалось добраться до стеллажа"
    ...
    if robot.coordinates_qr != route_to_station[-1]:
        robot.is_move_station = False
        return "Не удалось добраться до станции"
...
def simulate_move_return_and_home(robot, route_to_return, route_to_home, distances_to_return, distances_to_home):
    ...
    if robot.coordinates_qr != route_to_return[-1]:
        robot.is_move_back = False
        return "Не удалось вернуть стеллаж"
    ...
    if robot.coordinates_qr != route_to_home[-1]:
        robot.is_move_home = False
        return "Не удалось вернуться на базу"
...
def start_return_home_route(robot_id):
    ...
    return_data = wms.return_routes.get(robot.id)
    if not return_data:
